...
Automatic repository-wide detection is disabled. `input.css` scans the root `templates` directory explicitly and safelists the shared component/typography utilities. If a task introduces app-local templates, register that template directory with an `@source` directive in the same change; never scan documentation as CSS source.
