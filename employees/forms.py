...
from .models import Department, Employee, Position, Contract
...
class ContractForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "employee": forms.Select(
                attrs={
                    "class": "w-full rounded border border-outline-variant bg-surface px-3 py-2"
                }
            ),
...
class ContractForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if allowances_default is not None:

            if allowances_default < 0:
